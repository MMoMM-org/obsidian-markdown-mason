...
// catalog/entries/perplexity-web.ts
var perplexity_web_exports = {};
__export(perplexity_web_exports, {
  default: () => perplexity_web_default
});
module.exports = __toCommonJS(perplexity_web_exports);

// catalog/parsers/perplexityWeb.ts
var INLINE_LINK_RE = /(?<!!)\[([^\]]+)\]\(([^)]+)\)/g;
var SOURCES_BLOCK_RE = /^Sources\b/m;
var CITATIONS_BLOCK_RE = /^Citations:/m;
var QUELLEN_BLOCK_RE = /^Quellen\b/m;
var FOOTNOTE_DEF_RE = /^\[\^[^\]]+\]:/m;
function hasDisqualifyingBlock(input) {
  return SOURCES_BLOCK_RE.test(input) || CITATIONS_BLOCK_RE.test(input) || QUELLEN_BLOCK_RE.test(input) || FOOTNOTE_DEF_RE.test(input);
}
...
// catalog/scripts/perplexityWeb.ts
var perplexityWebScript = (ctx) => {
  if (!perplexityWeb.canParse(ctx.input)) return void 0;
...
// catalog/entries/perplexity-web.ts
var perplexity_web_default = {
  run: perplexityWebScript,
  paste: {
    canHandle: (input) => perplexityWeb.canParse(input),
    priority: 100
  }
};
