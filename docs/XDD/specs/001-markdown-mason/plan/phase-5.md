...
- [x] **T5.1 Script loader (ported)** `[activity: backend-api]`
...
- [x] **T5.2 Script runner — policy, timeout, fallback** `[activity: backend-api]`
...
- [x] **T5.3 Consent: disclosure modal + per-checksum/version store** `[activity: frontend-ui]`
...
- [x] **T5.4 Store: data.json manifest + per-device sidecar** `[activity: data-architecture]`
...
- [x] **T5.5 Vault import + command binding + paste invocation** `[activity: backend-api]`
...
- [x] **T5.6 The v0.1 Perplexity scripts** `[activity: backend-api]`
...
- [x] **T5.7 Phase Validation** `[activity: validate]`
  - Runtime + consent + store + scripts tests green; kill-switch and drift hard-block verified; lint + typecheck clean.

## Security review (defensive, end of Phase 5)
The trust model was reviewed and judged sound (escape guard, kill-switch, drift hard-block, atomicity, XSS-safe DOM, split-store consent all confirmed). Applied: SEC-001 (Enable-once no longer persists), SEC-003 (symmetric version check), SEC-004/005 (importScript rejects absolute + backslash-traversal paths), SEC-006 (documented bundled-enabled). **Deferred to v0.2** (gated on the user-script *execution* path + distribution, which are out of v0.1 scope):
- **SEC-002** — re-hash the on-disk script against the approved checksum at execution time (TOCTOU); must land before the imported-user-script execution path ships.
- **SEC-007** — verify Obsidian Sync does NOT replicate `device.json` (the 2-device consent-isolation spike already noted in the SDD); blocking for v0.2 multi-device.
- Also for Phase 7 (community compliance): CON-3 says `console.debug` only, but `loader.ts` (security warnings) and the script `logger` (warn/error) use `console.warn`/`console.error` — confirm acceptable or downgrade.
